...
int main()
{
    ...
    {
        printf("Numero invalido. Insira 1 ou 2: ");
        scanf("%d", &escolha);
    }
...
void opcao1()
{
    ...
    for (int i = 0; i < nos; i++)
    {
        int valor = (rand() % 100);               //numeros de 0 a 100;
    ...
    BOOLEAN ehAvl = verificaArvoreAVL(raiz);
...
void opcao2()
{
    ...
    //inserir valores que causem as 4 rotações
    int valoresInserir[] = {34, 65, 20, 14, 12, 21, 22, 23, 16, 18, 17}; //12 (RSD), 21 (RDD), 23 (RSE), 17 (RDE)
    ...
    BOOLEAN ehAvl = verificaArvoreAVL(raiz); //verificando árvore após inserção
    printf("\nArvore eh AVL? ");
